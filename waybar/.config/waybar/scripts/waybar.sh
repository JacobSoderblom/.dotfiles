...
# Find the line number for "output": "HDMI-A-1"
output_line=$(grep -n '"output": "HDMI-A-1"' "$CONFIG" | cut -d: -f1)
...
if [ "$(grep -w -B2 -A2 "output\": \"HDMI-A-1\"" $CONFIG | grep -w "layer" | awk '{printf $2}' | cut -d ',' -f1 | cut -d '"' -f2 | xargs)" == "top" ]; then
